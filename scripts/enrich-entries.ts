 ...
 * identical to stripMacrons in src/server/search.ts: this writes lemma_plain,
...
async function fetchLatinSection(lemma: string) {
	const plain = stripMacrons(lemma);
...
function toRow(candidate: Candidate): Row {
	...
	return {
		...
		lemmaPlain: stripMacrons(candidate.headword),
...
function chooseCandidate(
	...
	if (spec.lemma !== stripMacrons(spec.lemma)) {
...
for (const spec of specs) {
	try {
		...
		if (stripMacrons(row.lemma) !== stripMacrons(spec.lemma)) {
